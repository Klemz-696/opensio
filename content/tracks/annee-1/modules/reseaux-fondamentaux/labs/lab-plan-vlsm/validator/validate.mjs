...
/**
 * Validateur du lab "Plan d'adressage VLSM complet d'une entreprise multi-sites" (Niveau 2 - Fichiers)
...
export function validateVlsmPlan(workDir = '/work') {
  const csvPath = join(workDir, 'plan.csv');

  const checks = [
    { id: 'subnets_valid', passed: false, points: 0, maxPoints: 50, message: '' },
    { id: 'no_overlap', passed: false, points: 0, maxPoints: 30, message: '' },
    { id: 'doc_complete', passed: false, points: 0, maxPoints: 20, message: '' },
  ];
  ...
  const expectedServices = {
    siege: { minHosts: 500, requiredPrefix: 23 },
    agencenord: { minHosts: 120, requiredPrefix: 25 },
    agencesud: { minHosts: 60, requiredPrefix: 26 },
    dmzserveurs: { minHosts: 25, requiredPrefix: 27 },
    liaisonwan1: { minHosts: 2, requiredPrefix: 30 },
    liaisonwan2: { minHosts: 2, requiredPrefix: 30 },
  };

  const parsedEntries = new Map();
  const baseNetInt = ipToInt('172.16.0.0');
  const baseBroadcastInt = ipToInt('172.16.15.255');

  for (let i = 1; i < lines.length; i++) {
    const cols = lines[i].split(',').map((c) => c.trim());
    if (cols.length < 7) continue;

    const rawService = cols[0].toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]/g, '');
  ...
  for (const [srvKey, rules] of Object.entries(expectedServices)) {
    ...
    if (netInt === null || netInt < baseNetInt || netInt > baseBroadcastInt) {
      subnetsValid = false;
      subnetsErrors.push(`${entry.service} : l'adresse réseau ${entry.network} est hors du bloc 172.16.0.0/20.`);
      continue;
    }

    const mask = ((0xffffffff << (32 - entry.prefix)) >>> 0);
    if (((netInt & mask) >>> 0) !== netInt) {
      subnetsValid = false;
      subnetsErrors.push(`${entry.service} : ${entry.network} n'est pas une adresse réseau alignée sur un /${entry.prefix}.`);
    }
  }
  ...
  if (subnetsValid && subnetsErrors.length === 0) {
    ...
    checks[0].message = 'Découpage VLSM correct et adapté aux besoins de chaque service et liaison.';
  } else {
  ...
  if (!overlapFound && ranges.length === Object.keys(expectedServices).length) {
    checks[1].passed = true;
    checks[1].points = checks[1].maxPoints;
    checks[1].message = 'Aucun chevauchement d\'adresses entre les sous-réseaux.';
  } else {
    checks[1].message = overlapErrors.length > 0 ? overlapErrors.join(' ') : 'Sous-réseaux incomplets ou avec collisions.';
  }
...
if (process.argv[1] && process.argv[1].endsWith('validate.mjs')) {
  ...
  const verdict = validateVlsmPlan(workDir);
