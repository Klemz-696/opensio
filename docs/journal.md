...
- Découpage modulaire du schéma Prisma (`apps/api/prisma/schema/`) en 6 fichiers spécialisés :
  ...
  - `content.prisma` : Modèles `Track`, `Module`, `Lesson`, `Quiz`, `QuizQuestion`, enum `QuizQuestionKind`.
...
- Création et application de la migration initiale `20260824125010_init` sur le conteneur Docker PostgreSQL.
- Implémentation du script de seed idempotent `apps/api/prisma/seed.ts` avec hachage **Argon2id** (m=64 Mio, t=3, p=4) créant le compte administrateur (`admin@opensio.local`) et le compte étudiant de démonstration (`student@opensio.local`).
...
- Migration et Seed appliqués et testés avec succès sur PostgreSQL Docker.
