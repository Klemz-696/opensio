  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-6 left-6 z-30 lg:hidden flex items-center gap-2 px-4 py-2.5 rounded-full bg-slate-900/95 hover:bg-slate-800 text-sky-400 border border-slate-700/80 shadow-2xl backdrop-blur-md text-xs font-semibold transition-all cursor-pointer focus-visible:ring-2 focus-visible:ring-sky-400`}
      >
      ...
      <aside
        ...
        className={`fixed lg:sticky top-0 lg:top-24 right-0 lg:right-auto bottom-0 z-40 w-80 max-w-[85vw] bg-slate-900/95 lg:bg-slate-900/80 border-l lg:border border-slate-800 lg:rounded-2xl shadow-2xl flex flex-col transition-all duration-300 backdrop-blur-md h-full lg:h-[calc(100vh-8rem)] ${
      ...
      >
        ...
        {!isOpen && (
          <div className="hidden lg:flex flex-col items-center py-4 h-full justify-between">
            ...
            <div className="writing-vertical-rl text-[11px] font-bold uppercase tracking-wider text-slate-400 flex items-center gap-2 py-4">
              <span>Sommaire</span>
          ...
          <div className="flex flex-col h-full overflow-hidden">
            ...
            <div className="p-4 sm:p-5 border-b border-slate-800 flex items-start justify-between gap-3 shrink-0">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wider text-sky-400 mb-1">
                  <BookOpen className="w-3.5 h-3.5" />
                  <span className="truncate">{module.track.title}</span>
                </div>
                <Link
                  href={`/catalogue/${encodeURIComponent(module.slug)}`}
                  className="text-sm font-bold text-white hover:text-sky-300 transition-colors line-clamp-1 block"
                  title={module.title}
                >
                  {module.title}
                </Link>
                ...
                <div className="mt-3 space-y-1">
                  <div className="flex items-center justify-between text-[11px] text-slate-400">
                    ...
                    <span className="font-semibold text-sky-400">{progressPct}%</span>
                  </div>
              ...
              <button
                ...
                className="p-1.5 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800 transition-colors cursor-pointer shrink-0 mt-0.5"
              >
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center justify-between p-2.5 rounded-xl text-xs transition-all group ${
                      isCurrent
                        ? 'bg-sky-500/15 border border-sky-500/40 text-white font-semibold shadow-sm'
                        : isCompleted
                        ? 'bg-slate-900/40 hover:bg-slate-800/60 border border-emerald-500/20 text-slate-300'
                        : 'bg-slate-900/30 hover:bg-slate-800/50 border border-slate-800/60 text-slate-400 hover:text-slate-200'
                    }`}
                  >
                ...
                <div className="pt-3 mt-3 border-t border-slate-800/80 space-y-1.5">
                  ...
                  {module.quizzes.map((quiz) => (
                    <Link
                      ...
                      className="flex items-center justify-between p-2.5 rounded-xl text-xs bg-amber-500/5 hover:bg-amber-500/10 border border-amber-500/20 hover:border-amber-500/40 text-amber-300 transition-all"
                    >
                  ...
                  {module.labs.map((lab) => (
                    <Link
                      ...
                      className="flex items-center justify-between p-2.5 rounded-xl text-xs bg-emerald-500/5 hover:bg-emerald-500/10 border border-emerald-500/20 hover:border-emerald-500/40 text-emerald-300 transition-all"
                    >
                      <div className="flex items-center gap-2 truncate">
