          ...
          <ul className="space-y-2 text-xs text-slate-300">
            {lesson.prerequisites.map((prereq, idx) => (
              <li key={idx} className="flex items-center gap-2">
                <span className="font-mono text-amber-300 font-semibold">{prereq}</span>
              </li>
            ))}
          </ul>
        </div>
            ...
            {lesson.relatedLabs.map((lab) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[10px] text-purple-300 px-2 py-0.5 rounded bg-purple-500/10 border border-purple-500/20">
                    {labInfo.badge}
                  </span>
                </div>
              );
