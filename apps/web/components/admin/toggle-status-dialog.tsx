...
export function ToggleStatusDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 dark:bg-black/70 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="w-full max-w-md bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/80 rounded-2xl p-6 shadow-2xl space-y-6">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Utilisateur : <strong className="text-slate-800 dark:text-slate-200">{user.displayName}</strong>
              </p>
            </div>
          </div>
          <button
        ...
        {isSelf && isCurrentlyActive && (
          <div className="p-3.5 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-800 dark:text-amber-300 text-xs flex items-start gap-2.5">
            ...
            <div>
              <p className="font-semibold text-amber-900 dark:text-amber-200">Action interdite</p>
        ...
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-200 dark:border-slate-800">
          ...
          <button
            ...
            className={`px-5 py-2 rounded-xl text-white font-semibold text-xs transition-all shadow-md disabled:opacity-50 flex items-center gap-2 cursor-pointer ${isCurrentlyActive ? 'bg-rose-600 hover:bg-rose-500 shadow-rose-600/20' : 'bg-emerald-600 hover:bg-emerald-500 shadow-emerald-600/20'}`}
          >
            {isSubmitting ? (
              <>
