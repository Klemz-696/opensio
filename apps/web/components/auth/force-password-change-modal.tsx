...
export function ForcePasswordChangeModal() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-md animate-in fade-in duration-200">
      <div className="w-full max-w-md bg-slate-900 border border-slate-700/80 rounded-2xl p-6 sm:p-8 shadow-2xl space-y-6">
        <div className="text-center space-y-2">
          <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-400 mb-1">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="p-3 rounded-xl bg-slate-950/40 border border-slate-800 space-y-1.5 text-[11px]">
            <p className="font-semibold text-slate-400 mb-1">Exigences de sécurité :</p>
            <div className="grid grid-cols-2 gap-1.5 text-slate-400">
              <span className={`flex items-center gap-1.5 ${hasMinLength ? 'text-emerald-400' : ''}`}>
                {hasMinLength ? <CheckCircle2 className="w-3 h-3" /> : <XCircle className="w-3 h-3 text-slate-500" />}
                8 caractères min.
              </span>
              <span className={`flex items-center gap-1.5 ${hasUpper ? 'text-emerald-400' : ''}`}>
                {hasUpper ? <CheckCircle2 className="w-3 h-3" /> : <XCircle className="w-3 h-3 text-slate-500" />}
                1 majuscule
              </span>
              <span className={`flex items-center gap-1.5 ${hasLower ? 'text-emerald-400' : ''}`}>
                {hasLower ? <CheckCircle2 className="w-3 h-3" /> : <XCircle className="w-3 h-3 text-slate-500" />}
                1 minuscule
              </span>
              <span className={`flex items-center gap-1.5 ${hasNumber ? 'text-emerald-400' : ''}`}>
                {hasNumber ? <CheckCircle2 className="w-3 h-3" /> : <XCircle className="w-3 h-3 text-slate-500" />}
                1 chiffre
              </span>
              <span className={`flex items-center gap-1.5 ${hasSpecial ? 'text-emerald-400' : ''}`}>
                {hasSpecial ? <CheckCircle2 className="w-3 h-3" /> : <XCircle className="w-3 h-3 text-slate-500" />}
                1 caractère spécial
              </span>
              <span className={`flex items-center gap-1.5 ${passwordsMatch ? 'text-emerald-400' : ''}`}>
                {passwordsMatch ? <CheckCircle2 className="w-3 h-3" /> : <XCircle className="w-3 h-3 text-slate-500" />}
          ...
          <div className="flex items-center gap-3 pt-2">
            <button
              type="button"
              onClick={() => logout()}
              className="px-4 py-2.5 rounded-xl border border-slate-700 bg-slate-800/80 hover:bg-slate-800 text-slate-300 text-xs font-medium transition-colors flex items-center gap-1.5 cursor-pointer"
            >
