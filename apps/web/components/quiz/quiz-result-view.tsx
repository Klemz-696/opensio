...
export function QuizResultView({
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div
        className={`glass-panel rounded-3xl p-8 sm:p-10 border shadow-md dark:shadow-2xl relative overflow-hidden ${
          isPassed
            ? 'bg-gradient-to-b from-emerald-500/10 via-white to-slate-50 dark:from-emerald-950/40 dark:via-slate-900/90 dark:to-slate-950 border-emerald-500/30'
            : 'bg-gradient-to-b from-rose-500/10 via-white to-slate-50 dark:from-rose-950/40 dark:via-slate-900/90 dark:to-slate-950 border-rose-500/30'
        }`}
      >
        <div className="flex flex-col md:flex-row items-center justify-between gap-8">
          ...
          <div className="flex flex-col items-center justify-center p-6 rounded-2xl bg-white/80 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 text-center min-w-[200px] shrink-0 shadow-sm">
            ...
            <div className="text-xs text-slate-500 dark:text-slate-400 mt-2 flex items-center gap-1.5 font-medium">
        ...
        {/* Boutons d'action */}
        <div className="mt-8 pt-6 border-t border-slate-200 dark:border-slate-800/80 flex flex-wrap items-center justify-between gap-4">
          <Link
            href={`/catalogue/${encodeURIComponent(moduleSlug)}`}
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 text-xs font-semibold transition-colors border border-slate-200 dark:border-slate-700"
          >
      ...
      <div className="space-y-6">
        ...
        {result.questions.map((question, index) => (
          <div
            ...
            className={`glass-panel rounded-2xl p-6 sm:p-8 border shadow-sm dark:shadow-lg transition-all ${
              question.isCorrect
                ? 'bg-slate-50 dark:bg-slate-900/60 border-emerald-500/30'
                : 'bg-slate-50 dark:bg-slate-900/60 border-rose-500/30'
            }`}
          >
              ...
              <div className="flex items-center gap-2">
                ...
                {!question.isCorrect && (
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-500/10 hover:bg-indigo-500/20 dark:bg-indigo-600/20 dark:hover:bg-indigo-600/30 text-indigo-700 dark:text-indigo-300 hover:text-indigo-800 dark:hover:text-indigo-200 border border-indigo-500/40 text-xs font-medium transition-all cursor-pointer shadow-sm"
                  >
            ...
            <div className="mb-6 text-slate-800 dark:text-slate-200 text-sm sm:text-base">
              <MarkdownRenderer content={question.prompt} />
            </div>

            {/* Explication pédagogique statique existante */}
            {question.explanation && (
              <div className="mt-4 p-4 sm:p-5 rounded-xl bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 text-xs sm:text-sm text-slate-700 dark:text-slate-300">
                <div className="flex items-center gap-2 text-amber-600 dark:text-amber-400 font-semibold text-xs mb-2">
