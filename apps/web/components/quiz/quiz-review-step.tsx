  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="glass-panel rounded-2xl p-6 sm:p-8 border border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 shadow-sm dark:shadow-xl">
        ...
        {unansweredCount > 0 && (
          <div className="mt-4 p-3.5 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-800 dark:text-amber-200 text-xs flex items-start gap-2.5">
      ...
      {error && (
        <div className="p-4 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-700 dark:text-rose-300 text-sm flex items-start gap-3">
          ...
          <div className="flex-1">
            <p className="font-semibold text-rose-800 dark:text-rose-200">Erreur de soumission</p>
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-xs font-semibold text-sky-600 dark:text-sky-400 hover:text-sky-700 dark:hover:text-sky-300 transition-colors cursor-pointer border border-slate-200 dark:border-slate-700"
                >
              ...
              {/* Énoncé résumé */}
              <p className="text-xs sm:text-sm text-slate-700 dark:text-slate-300 mb-3 line-clamp-2">
                {question.prompt}
              </p>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <ul className="space-y-1">
                      {selectedChoices.map((choice) => (
                        <li
                          key={choice.id}
                          className="flex items-center gap-2 text-xs text-slate-800 dark:text-slate-200 bg-white dark:bg-slate-800/50 px-3 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700/60"
                        >
      ...
      <div className="sticky bottom-4 z-20 glass-panel rounded-2xl p-4 sm:p-5 border border-slate-200 dark:border-slate-800 bg-white/90 dark:bg-slate-900/90 shadow-xl dark:shadow-2xl backdrop-blur-md flex flex-wrap items-center justify-between gap-4">
        <button
          ...
          disabled={isSubmitting}
          className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 text-xs sm:text-sm font-semibold transition-colors cursor-pointer border border-slate-200 dark:border-slate-700"
        >
