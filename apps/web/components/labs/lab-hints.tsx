      ...
      {unlockedHints.length > 0 && (
        <div className="space-y-3 mb-4">
          {unlockedHints.map((hint) => (
            <div
              key={hint.index}
              className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-200"
            >
              <div className="flex items-center gap-2 font-bold text-xs text-amber-300 uppercase tracking-wider mb-1.5">
                <Lightbulb className="w-4 h-4 text-amber-400" />
                <span>Indice {hint.index} (Coût : -{hint.costPercent}%)</span>
              </div>
              <p className="text-sm text-amber-100 leading-relaxed font-normal">
                {hint.text}
              </p>
            </div>
          ))}
