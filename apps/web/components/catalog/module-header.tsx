...
export function ModuleHeader({ module }: ModuleHeaderProps) {
  ...
  return (
    <div className="glass-panel rounded-2xl p-8 mb-8 border border-slate-700/60 bg-gradient-to-b from-slate-900/90 to-slate-950/90">
      <div className="flex flex-wrap items-center gap-3 mb-4">
        <span className="text-xs font-semibold px-3 py-1 rounded-full bg-slate-800 border border-slate-700 text-slate-300 flex items-center gap-1.5">
        ...
        <span className="text-xs text-slate-400 flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-800/60 border border-slate-700/60">
      ...
      {module.competencyRefs && module.competencyRefs.length > 0 && (
        <div className="flex items-center gap-2 pt-4 border-t border-slate-800/80">
          <span className="text-xs text-slate-400 font-medium">Référentiel BTS SIO :</span>
          <div className="flex flex-wrap gap-1.5">
            {module.competencyRefs.map((comp) => (
              <span
                key={comp}
                className="text-xs font-mono font-semibold px-2.5 py-0.5 rounded bg-sky-500/10 border border-sky-500/30 text-sky-300"
              >
                {comp}
              </span>
            ))}
