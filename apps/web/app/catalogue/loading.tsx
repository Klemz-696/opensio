    ...
    <div role="status" aria-busy="true" aria-label="Chargement du catalogue de formation" className="animate-pulse">
      ...
      <div className="mb-10">
        <div className="flex items-center gap-3 mb-3">
          <div className="w-10 h-10 rounded-xl bg-slate-800 border border-slate-700/50" />
          <div className="h-9 bg-slate-800 rounded-lg w-80 sm:w-96 max-w-full" />
        </div>
      ...
      <div className="mb-14">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map((cardId) => (
            <div
              key={cardId}
              className="glass-panel rounded-2xl p-6 border border-slate-800 flex flex-col justify-between h-72"
            >
              <div>
                {/* Badges en-tête */}
