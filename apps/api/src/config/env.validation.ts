  ...
  .refine(
    (data) => {
      if (data.NODE_ENV === 'test') {
        ...
        return isTestDb && !isDevDb;
      }
      return true;
    },
    {
      message:
        'Garde-fou environnement de test : en mode NODE_ENV=test, DATABASE_URL doit pointer vers une base de données de test dédiée (ex: opensio_test) et ne doit jamais cibler la base de développement.',
      path: ['DATABASE_URL'],
    }
  );
